...
export function MetricCard({
  ...
  return (
    <Card className={cn("border-white/10 bg-white/[0.04] p-5", className)}>
      ...
      {!reverse ? <p className={cn("mt-3 font-display text-4xl", valueClassName)}>{value}</p> : null}
      {detail ? <p className={cn("mt-2 text-sm text-muted-foreground", detailClassName)}>{detail}</p> : null}
    </Card>
  );
}
